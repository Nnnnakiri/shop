...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('getGoodInfo', this.$route.params.skilId)
  },
  computed: {
  ...
  methods: {
    ...
    changeSkuNum(event) {
      ...
      let newSkuNum = event.target.value * 1
      if (isNaN(newSkuNum) || newSkuNum < 1) {
        this.skuNum = 1;
      } else {
        this.skuNum = parseInt(newSkuNum);
      }
    ...
    async addShopCar() {
      try {
        // 派发 actions  需要将产品ID 和 产品数量发送过去
        await this.$store.dispatch('GetOrUpdateShopCar', {
          skuId: this.$route.params.skilId, skuNum: this.skuNum
        });
        // 成功后跳转路由  并将产品信息通过本地存储存起来
